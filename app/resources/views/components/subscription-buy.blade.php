@if($subscription->periods()->exists())
    <div class="block-lk1__button">
        ...
        <a href="#" data-fancybox data-src="#modal-subscription-payment" data-service-id="{{ $subscription->id }}" data-service-type="{{ \App\Models\Subscription::class }}" class="item__buy block-lk1__button-buy">@if($user->subscriptions()->where("id", $subscription->id)->exists())ПРОДЛИТЬ ПОДПИСКУ@elseКУПИТЬ ПОДПИСКУ@endif</a>
    </div>
@endif
